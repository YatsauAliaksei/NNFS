...
//            charToIdxMap.put(validCharacters[i], i);
                IntStream.range(0, validCharacters.length).boxed()
                        .collect(toMap(i -> validCharacters[i], Function.identity()));

        //Load file and convert contents to a char[]
        List<String> lines = Files.readAllLines(new File(textFilePath).toPath(), textFileEncoding);
        lines = excludeComments(commentChars, lines);
...
//            maxSize += s.length();
    ...
    private List<String> excludeComments(String commentChars, List<String> lines) {
